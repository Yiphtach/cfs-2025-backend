...
describe('Authentication API', () => {
    ...
    it('should not allow duplicate registration', async () => {
        await request(app).post('/api/auth/register').send({ username: 'testuser', email: 'test@example.com', password: 'password123' });
        const response = await request(app).post('/api/auth/register').send({ username: 'testuser', email: 'test@example.com', password: 'password123' });
...
// Unit Tests - Fight Simulation
describe('Fight Simulation API', () => {
    it('should simulate a fight and return a winner', async () => {
        const response = await request(app)
            .post('/api/fights/simulate')
            .send({ userId: '60d0fe4f5311236168a109ca', fighter1Id: '1', fighter2Id: '2' });
        expect(response.status).toBe(200);
